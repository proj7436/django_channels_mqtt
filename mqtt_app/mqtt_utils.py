...
def on_message(mqtt_client, userdata, msg):
    global run_python_program
    
    try:
        # Giải mã hex string từ dạng byte sang chuỗi
        hex_str = msg.payload.decode("utf-8")
    except UnicodeDecodeError:
        # Nếu không thể giải mã từ utf-8, có thể đây không phải là hex string,
        # do đó chuyển đổi từ byte sang hex string
        hex_str = msg.payload.hex()

    # Gọi hàm run_python_program
    print(run_python_program(msg.topic, msg.qos, hex_str))
